  ...
  if (to.name === "updateShare") {
    ...
    if (
      ...
      if (to.params.fileInfo === undefined) {
        ...
        await addWatchCount(to.params.id as string);
        await addBrowseHistory(to.params.id as string);
        to.params.fileInfo = JSON.parse(to.params.fileInfo as string);
        return 1;
      }
    } else {
    ...
    if (to.params.id != "" && to.params.id != null && to.params.id != null) {
      if (to.params.projectInfo != undefined && to.params.projectInfo != null) {
        ...
        if (
          (to.params.projectInfo as any).creator != store.state.user.email &&
          (to.params.projectInfo as any).isPublic === false
        ) {
          return -1;
        }
        ...
        if (data != null) {
          if ((data as any).code === 0) {
            ...
            if (
              (to.params.projectInfo as any).creator !=
